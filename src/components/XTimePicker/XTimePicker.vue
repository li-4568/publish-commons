<template>
  <div :class="pickerWrapperClasses">
    ...
    <!-- 错误提示信息 -->
    <div v-if="computedErrorMessage" class="xtimepicker-error-message">
...
<script setup lang="ts">
  /**
   * XTimePicker 组件
   * 时间选择器，基于 Ant Design Vue TimePicker 封装
   */
  ...
  const emit = defineEmits<{
    ...
    (e: 'change', value: string | null, time: dayjs.Dayjs | null): void
  ...
  const pickerWrapperClasses = computed(() => {
    const classes = ['xtimepicker-wrapper']

    if (props.size) {
      classes.push(`xtimepicker-${props.size}`)
    }

    if (props.disabled) {
      classes.push('xtimepicker-disabled')
    }

    if (props.required) {
      classes.push('xtimepicker-required')
    }

    if (computedErrorMessage.value) {
      classes.push('xtimepicker-has-error')
    }
   ...
   * 处理值改变事件
   */
  const handleChange = (value: dayjs.Dayjs | string | null, timeString: string) => {
    internalValue.value = timeString || undefined
    emit('update:modelValue', timeString || null)
    emit('update:value', timeString || null)
    emit('change', timeString || null, value as dayjs.Dayjs | null)

    // 根据触发时机决定是否校验
    if (props.validateOnChange && props.validateTrigger?.includes('change')) {
      runValidation(timeString || undefined)
    }
  }
...
<style scoped lang="less">
  .xtimepicker-wrapper {
    ...
    &.xtimepicker-has-error {
      .ant-picker {
        border-color: @error-color;
      }
    }

    &.xtimepicker-disabled {
      opacity: 0.6;
    }

    &.xtimepicker-required {
